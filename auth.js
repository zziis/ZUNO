class ZonoAuth {
  ...
  async init() {
    ...
    // Supabase persists the session in localStorage. Restore it before showing
    // the login screen so Refresh/reopening the site does not flash auth UI.
    ...
    this.client.auth.onAuthStateChange(async (event, session) => {
      ...
      // Only expose the login screen when there is really no saved session
      // or the user explicitly signed out.
      if (this.initialized && (event === 'SIGNED_OUT' || event === 'INITIAL_SESSION')) {
        this.user = null;
        this.profile = null;
