-- ZONO 15 THEMES UPDATE
-- Run ONCE in Supabase SQL Editor.
-- Keeps existing purchases and adds the 10 new paid themes.
...
begin
  ...
  v_price:=case p_theme_key
    ...
    when 'electric' then 900
    when 'cyber' then 1000
    when 'aurora' then 1100
    when 'emerald' then 1200
    when 'ocean' then 1300
    when 'ruby' then 1400
    when 'desert_gold' then 1500
    when 'ice' then 1600
    when 'galaxy' then 1800
    when 'imperial' then 2000
    else null
  end;

  if v_price is null then
    raise exception 'هذا الثيم مجاني أو غير موجود';
  end if;

  if exists(
    select 1 from public.zono_theme_ownership
    where user_id=auth.uid() and theme_key=p_theme_key
  ) then
  ...
  set feathers=coalesce(feathers,0)-v_price
  where id=auth.uid();

  insert into public.zono_theme_ownership(user_id,theme_key)
  values(auth.uid(),p_theme_key)
...
begin
  ...
  if p_theme_key not in (
    'classic_night','daylight','lunar','solar','royal_bird',
    'electric','cyber','aurora','emerald','ocean',
    'ruby','desert_gold','ice','galaxy','imperial'
  ) then
