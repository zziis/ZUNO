class ZonoApp {
    ...
    async init() {
        ...
        try {
            const logged = await window.zunoAuth.init();
            if (logged) {
    ...
    async syncUserFromSupabase() {
        const p = window.zunoAuth?.profile;
        const u = window.zunoAuth?.user;
        if (!p || !u) return;

        const inventory = await window.zunoAuth.featherInventory();
    ...
    async handleRegister(event) {
        ...
        this.setAuthMessage('');
        try {
            const data = await window.zunoAuth.register({
    ...
    async logout() {
        ...
        try { await window.zunoAuth.logout(); } catch (_) {}
        this.currentUser = null;
        this.showAuthModal();
        this.authTab('login');
    ...
    async buyItem(itemId, price) {
        ...
        try {
            ...
            await window.zunoAuth.loadProfile(window.zunoAuth.user);
            await this.syncUserFromSupabase();
    ...
    async saveEditedProfile() {
        ...
        try {
            if (nameInput && nameInput.value.trim() && nameInput.value.trim() !== this.currentUser.displayName) {
                await window.zunoAuth.updateDisplayName(nameInput.value.trim());
            }
